	    ...
	    <el-table-column
	        ...
	        <template slot-scope="scope" >
	            ...
	            <!--<span @click="updateStatus(scope.row.operatorName,scope.row.id,scope.row.status)" :class="!(scope.row.status=='ENABLED')?'suc':'fail'" style="margin-right: 10px;cursor: pointer;">
	           	...
	           	<el-button v-if="!roleHasPermissions['permission_manage_role_manage_update']" @click="UpdateRole(scope.row.id,scope.row.roleName,scope.row.remark)"  type="warning" size="mini">修改</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      UpdateRole(_id,_roleName,_remark){//修改点击事件
      	this.formUpdateRole.id = _id;
      	this.formUpdateRole.roleName = _roleName;
      	this.formUpdateRole.remark = _remark;
