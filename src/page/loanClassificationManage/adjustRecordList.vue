  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="调整前五级分类：" prop="loanClassificationBefore">
          <el-select v-model="listQuery.loanClassificationBefore" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
        ...
        <el-form-item label="调整后五级分类：" prop="loanClassificationAfter">
          <el-select v-model="listQuery.loanClassificationAfter" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
            <el-option v-if="loanClassificationTypeList" v-for="(val,key) in loanClassificationTypeList" :label="val" :value='key' :key='key'>
            </el-option>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        prop="loanClassificationBeforeText"
        label="调整前五级分类">
      </el-table-column>
      
      <el-table-column
        prop="loanClassificationAfterText"
        label="调整后五级分类">
      </el-table-column>
      ...
      <el-table-column
        ...
        label="剩余未结清金额">
        <template slot-scope="scope" >
          <div>{{scope.row.unclearAmount}} 元</div>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,collateralRegisterStatus,loanClassificationType} from '@/api/loanClassificationManage/adjustRecordList'
