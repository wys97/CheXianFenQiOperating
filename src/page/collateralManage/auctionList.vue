  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="状态：" prop="collateralStatus">
          <el-select v-model="listQuery.collateralStatus" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        prop="collateralStatusText"
        width="80"
        label="状态">
        <template slot-scope="scope" >
          <span :class="scope.row.collateralStatus==='IN'?'suc':'draw' ">
          	{{scope.row.collateralStatusText}}
         	</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            <router-link :to="'auctionDetail/' + scope.row.collateralId" class="router-link">查看</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,collateraCheckStatus} from '@/api/collateralManage/auctionList'
