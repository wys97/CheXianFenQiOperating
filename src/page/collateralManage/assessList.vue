  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="押品类型：" prop="collateralType">
          <el-select v-model="listQuery.collateralType" style="width:150px;">
        ...
        <el-form-item label="法定代表人身份证：" prop="repreIdCardNo">
          <el-input v-model="listQuery.repreIdCardNo" style="width:130px;"></el-input>
        </el-form-item>
        <el-form-item label="状态：" prop="registerStatus">
          <el-select v-model="listQuery.registerStatus" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" >
          ...
          <div>{{scope.row.repreIdCardNo}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        label="状态">
        <template slot-scope="scope" >
        	<span :class="scope.row.registerStatus==='EVALUATING'?'war':scope.row.registerStatus==='CHECKING'?'suc':scope.row.registerStatus==='REJECT_CHECK'?'fail':'draw' ">
          	{{scope.row.registerStatusText}}
         	</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            <router-link :to="'assessDetail/' + scope.row.collateralId" class="router-link">查看</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,collateraAssessStatus} from '@/api/collateralManage/assessList'
  ...
  export default {
    ...
    methods: {
      ...
      getRegisterStatusList (){//申请登记-状态下拉
      	apiCommon.collateraAssessStatus ().then(res => {
          if (res.data.code === '200') {
