  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        label="状态">
        <template slot-scope="scope" >
          <span>{{scope.row.registerStatusText}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            <router-link :to="'checkDetail/' + scope.row.collateralId" class="router-link">查看</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,collateraCheckStatus } from '@/api/collateralManage/checkList'
