      ...
      <div class="flex-item" style="text-align: right;padding-top: 0;">
        ...
        <el-button
          :type="checkHeadData.status=='REVIEW_SUCCESS'?'success':
	      		checkHeadData.status=='BACK'?'warning':
	      		checkHeadData.status=='REJECT'?'danger':
	      		checkHeadData.status=='RETURNED'?'warning':
            checkHeadData.status=='PENDING'?'warning':''"
          size="mini"
        >{{checkHeadData.statusText}}</el-button>
      </div>
    </div>
    <div class="content">
      <div class="flex-item" style="position: relative;">
        ...
        <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
          <el-tab-pane class="tab_content" label="尽调检查" name="first">
            <div class="tab-content">
              <template >
                <Verify :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="true"></Verify>
              </template>
            </div>
          </el-tab-pane>
          <el-tab-pane label="评级打分卡" name="second">
            <div class="tab-content">
              <template>
                <Scorecard :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="true"></Scorecard>
              </template>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="复审授信结论" name="four">
            <div class="tab-content">
              <template v-if="recheckData">
                <Recheck
                  :api-url="'recheckDetail'"
                  :supplier-id="supplierId"
                  :readOnly="(checkHeadData.status=='PENDING'||checkHeadData.status==='RETURNED')?false:true"
        ...
        <ul style="margin-top: 20px;">
          ...
          <li>
            <div class="load_box_item">
              ...
              <div class="load_box_item_btns">
                <a
                  :href="'/admin/check/check_enterprise_data_export?supplierId='+supplierId"
                  target="_blank"
                >
          ...
          <li>
            <div class="load_box_item">
              ...
              <div class="load_box_item_btns">
                <a
                  :href="'/admin/check/download_image_file?supplierId='+supplierId"
                  target="_blank"
                >
                  <el-button size="mini" type="primary">下载</el-button>
                </a>
  ...
  methods: {
    ...
    handleClick(tab) {
      if (tab.index === "0") {
        ...
        this.getQuerySignConclusion(); 
      } else if (tab.index === "3") {
        this.getQueryRecheckData();
      }
    },
    init() {
    ...
    getQuerySignConclusion() {
      //初审授信结论-查询
      recheckDetail
        .signConclusion(this.supplierId)
        .then(res => {
          if (res.data.code === "200") {
    ...
    //      }).catch(() => {})
    //    },
    getSignEnterpriseStrengthList() {
      //企业实力和业绩单选数据
      recheckDetail
        .signEnterpriseStrength(this.supplierId)
        .then(res => {
          if (res.data.code === "200") {
