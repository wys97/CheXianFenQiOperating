    ...
    <div class="top">
      	...
      	<div class="flex-item" style="text-align: right;padding-top: 0;">
	      	...
	      	<el-button 
	      		...
	      		checkHeadData.status=='RETURNED'||checkHeadData.status=='PENDING'?'warning':''" size="mini">
      ...
      <div class="flex-item" style="position: relative;">
      	...
      	<el-tabs v-model="activeName2" type="card"  @tab-click="handleClick">
            ...
            <div class="tab-content">
            	<template>
		              <Verify :api-url="'firstTrialDetail'" :supplier-id="supplierId">
		              </Verify>
		          </template>
            </div>
	        </el-tab-pane>
        	<el-tab-pane label="评级打分卡" name="second">
		              ...
		              <Scorecard :api-url="'firstTrialDetail'" :supplier-id="supplierId">
		              </Scorecard>
		          </template>
						</div>
					</el-tab-pane>
...
//	  		this.verieyCheck();//尽调检查
	    ...
	    getQueryCheckHead (){//押品管理-信息登记-头部信息
	      	firstTrialDetail.checkHead(this.supplierId).then(res => {
      ...
      getQuerySignConclusion(){//授信结论-查询
      	firstTrialDetail.signConclusion(this.supplierId).then(res => {
          if (res.data.code === '200') {
...
//    	firstTrialDetail.scorecardQuery(this.supplierId).then(res => {
//        if (res.data.code === '200') {
...
//        	}
        ...
        this.$confirm('确认通过初审？', '操作提示', {
            ...
            firstTrialDetail.checkPass(this.supplierId).then(res => {
		          if (res.data.code === '200') {
        ...
        firstTrialDetail.checkReject(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
        ...
        firstTrialDetail.checkReturned(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
