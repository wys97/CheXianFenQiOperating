		...
		<div class="top">
			...
			<div class="flex-item" style="text-align: right;padding-top: 0;">
				...
				<el-button 
					...
					checkHeadData.status=='REJECT'?'danger':'success'" size="mini">
				...
				<el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
					<el-tab-pane label="评级打分卡" name="first">
						<div class="tab-content">
							<template>
								<Scorecard :api-url="'finalsecondDetail'" :supplier-id="supplierId" :readOnly="true">
								</Scorecard>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane class="tab_content" label="尽调检查" name="second">
						<div class="tab-content">
							<template v-if="verifyShow">
								<Verify :api-url="'finalsecondDetail'" :supplier-id="supplierId" :readOnly="true">
								</Verify>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane label="初审结论" name="three">
						<div class="tab-content">
							<template v-if="checkData">
								<Check :api-url="'finalsecondDetail'" :supplier-id="supplierId" :readOnly="true" :check-data="checkData">
								</Check>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane label="复审结论" name="four">
						<div class="tab-content">
							<template v-if="recheckData">
								<Recheck :api-url="'finalsecondDetail'" :supplier-id="supplierId" :readOnly="true" :recheck-data="recheckData">
								</Recheck>
							</template>
						</div>
					</el-tab-pane>
							...
							<div class="info-list">
								<el-form ref="verifyFirmInfoForm" label-width="170px" size="mini" label-position="right">
									...
									<el-form-item label="备注说明：">
										<el-input type="textarea" :rows="4" :disabled="true" v-model="reviewRemark">
										</el-input>
									</el-form-item>
								</el-form>
							</div>
							<div v-if="false" class="info-list">
					...
					<el-tab-pane label="终审二审备注" name="six">
						<div class="tab-content">
							<div class="info-list">
								<el-form ref="verifyFirmInfoForm" label-width="170px" size="mini" label-position="right">
									<div class="lh_title">终审二审备注说明</div>
									<el-form-item label="备注说明：">
										<el-input type="textarea" :rows="4" v-model="finalsecondRemark">
										</el-input>
									</el-form-item>
								</el-form>
							</div>
							<div class="info-list">
								<div style="text-align: right;margin-top: 10px;">
									<el-button @click="doFinalsecondSave" type="primary">保存</el-button>
								</div>
							</div>
						</div>
					</el-tab-pane>
				...
				<ul>
					...
					<li v-if="checkHeadData.status==='PENDING'">
						<div class="fk_btn_box">
							<el-button @click="doPass" v-if="!roleHasPermissions['finance_manage_register_list_audit']" type="success">终审二审通过</el-button>
						</div>
					</li>
					<li v-if="checkHeadData.status==='PENDING'">
						<div class="fk_btn_box">
							<el-button @click="doReject " v-if="!roleHasPermissions['finance_manage_register_list_audit']" type="danger">终审二审拒绝</el-button>
						</div>
					</li>
					<!--<li>
						<div class="fk_btn_box">
		...
		methods: {
			...
			handleClick(tab) {
				if(tab.index === '0') { //评级打分卡
					...
					this.getQueryFinalsecondDetail();//终审一审备注查询
				}
			},
			init() {
			...
			getQueryCheckHead() { //押品管理-信息登记-头部信息
				finalsecondDetail.checkHead(this.supplierId).then(res => {
			...
			getQuerySignConclusion() { //初审授信结论-查询
				finalsecondDetail.signConclusion(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			getQueryRecheckData() { //复审授信结论-查询
				finalsecondDetail.recheckDetail(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			//    	finalsecondDetail.scorecardQuery(this.supplierId).then(res => {
			//        if (res.data.code === '200') {
			...
			getQueryReviewDetail(){//终审一审备注查询
				finalsecondDetail.reviewDetail(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doReviewSave(){//终审一审备注保存
				...
				finalsecondDetail.reviewSave(data).then(res => {
					if(res.data.code === '200') {
			...
			getQueryFinalsecondDetail(){//终审二审备注查询
				finalsecondDetail.finalsecondDetail(this.supplierId).then(res => {
					if(res.data.code === '200') {
						this.finalsecondRemark = res.data.data.remark;
					} else {
						this.$message.error(res.data.message)
					}
				}).catch(() => {})
			},
			doFinalsecondSave(){//终审二审备注保存
				let data={
					remark:this.finalsecondRemark,
					supplierId:this.supplierId
				}
				finalsecondDetail.finalsecondSave(data).then(res => {
					if(res.data.code === '200') {
						this.$message.success(res.data.message)
					} else {
						this.$message.error(res.data.message)
			...
			getSignEnterpriseStrengthList() { //企业实力和业绩单选数据
				finalsecondDetail.signEnterpriseStrength(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doPass() { //通过
				this.$confirm('确认通过终审二审？', '操作提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					finalsecondDetail.checkPass(this.supplierId).then(res => {
						if(res.data.code === '200') {
							this.$message.success(res.data.message)
							this.getQueryCheckHead(); //头部信息
						} else {
			...
			approveFinal() { //确认拒绝提交
				...
				finalsecondDetail.checkReject(data).then(res => {
					if(res.data.code === '200') {
						this.$message.success(res.data.message)
