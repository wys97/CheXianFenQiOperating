	...
	<div class="page-container" ref="page-container" id="page-container">
		...
		<div class="filter-container" ref="filter-container" id="filter-container">
			<el-form :inline="true" :model="listQuery" ref="listQueryForm">
				<el-form-item label="评分卡编号：" prop="cardNo">
					<el-input v-model="listQuery.cardNo" style="width:120px;"></el-input>
				</el-form-item>
				<el-form-item label="评分卡名称：" prop="cardName">
					<el-input v-model="listQuery.cardName" style="width:120px;"></el-input>
				</el-form-item>
				<el-form-item label="应用场景：" prop="sceneType" class="query-select">
					<el-select v-model="listQuery.sceneType" style="width:150px;">
						<el-option label="全部" value=""></el-option>
						<el-option v-for="(val,key) in cardSceneTypeList" :label="val" :value='key' :key="key">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="状态" prop="status" class="query-select">
					<el-select v-model="listQuery.status" style="width:150px;">
						<el-option label="全部" value=""></el-option>
						<el-option v-for="(val,key) in capitalStatusListData" :label="val" :value='key' :key="key">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item>
			...
			<div class="table_title_box">
				<span>评分卡管理</span>
		    	<router-link :to="'scoreCardAdd'" class="router-link">
		...
		<el-table :data="list" @current-change="handleSelectionChange" highlight-current-row :max-height=tableMaxHeight>
			...
			<el-table-column prop="cardNo" label="评分卡编号" >
				<template slot-scope="scope">
					<div>{{scope.row.cardNo}}</div>
				</template>
			</el-table-column>
			<el-table-column prop="cardName" label="评分卡名称">
			</el-table-column>
			<el-table-column prop="sceneTypeText" label="应用场景">
			</el-table-column>
		...
		<el-table-column prop="isOverdue" label="是否逾期" min-width="80">
			...
			<el-table-column prop="cardId" label="操作" width="122" align="center">
				<template slot-scope="scope" v-if="!roleHasPermissions['capital_manager_capital_list_list']">
					<router-link :to="'scoreCardDetail/' + scope.row.cardId" class="router-link">查看</router-link>
				</template>
			</el-table-column>
			...
			<el-form ref="capitalDetailForm" label-width="190px" size="mini" label-position="right">
				<div class="info-list">
					...
					<el-form-item label="资方简称：">
						<el-input v-model="capitalAddForm.capitalShortName" style="width: 300px;">
						</el-input>
					</el-form-item>
					<el-form-item label="资方类型：">
						<template slot-scope="scope">
							...
							<el-select v-model="capitalAddForm.capitalType" placeholder="请选择">
								<el-option v-for="(val,key) in capitalTypeList" :key="key" :label="val" :value="key"></el-option>
							</el-select>
						</template>
					</el-form-item>
					<el-form-item label="资方企业名称：">
						<el-input v-model="capitalAddForm.capitalName" style="width: 300px;">
						</el-input>
					</el-form-item>
					<el-form-item label="社会统一信用代码：">
						<el-input v-model="capitalAddForm.businessLicenseNo" style="width: 300px;">
						</el-input>
					</el-form-item>
					<el-form-item label="合作日期：">
						<el-date-picker v-model="capitalAddForm.cooperateDate" type="date" placeholder="选择日期">
						</el-date-picker>
					</el-form-item>
					<el-form-item label="经办人姓名：">
						<el-input v-model="capitalAddForm.repreName" style="width: 300px;">
						</el-input>
					</el-form-item>
					
					<el-form-item label="经办人身份证：">
						<el-input v-model="capitalAddForm.repreIdcardNo" style="width: 300px;">
						</el-input>
					</el-form-item>
					
					<el-form-item label="经办人手机号：">
						<el-input v-model="capitalAddForm.repreMobile" style="width: 300px;">
						</el-input>
					</el-form-item>
					...
					<el-form-item label="备注：">
						<el-input type="textarea" :rows="4" v-model="capitalAddForm.remark">
						</el-input>
					</el-form-item>
				</div>
			</el-form>
...
<script>
	...
	import { getListData, capitalStatusList,capitalType,capitalSave } from '@/api/scoreCardManage/scoreCardList'
	import { cardSceneType} from '@/api/common'
	...
	export default {
		...
		created() {
			...
			this.getCardSceneTypeList();//获取评分卡应用场景下拉
		},
		mounted() {
			setTimeout(() => {
		...
		methods: {
			...
			saveCapitalDetail() { //保存资金方信息
				...
				capitalSave(data).then(res => {
					if(res.data.code === '200') {
//						this.getList();//资金方信息---------查询
						this.$router.push({path:'/capticalManage/capticalDetail/'+res.data.data.capitalId})
			...
			getCardSceneTypeList() { //获取评分卡应用场景下拉
