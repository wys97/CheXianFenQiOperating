  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="法定代表人手机号" prop="repreMobile">
        ...
        <el-form-item label="资方" class="query-select" prop="capitalId">
          <el-select v-model="listQuery.capitalId" style="width:110px;">
            <el-option label="全部" value="">
            </el-option>
            <el-option v-if="capitalEnterpriseListData" 
            	v-for="item in capitalEnterpriseListData" 
        ...
        <el-form-item label="进件对接状态：" class="query-select" prop="status">
          <el-select v-model="listQuery.status" style="width:110px;">
            <el-option label="全部" value="">
            </el-option>
            <el-option v-if="supplystatusListData" 
            	v-for="(val,key) in supplystatusListData" 
            	:label="val" 
            	:value='key' 
            	:key='key'>
            </el-option>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            <router-link :to="'intoButtDetail/' + scope.row.supplierId" class="router-link">查看</router-link>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getListData,getRegisterStatusList,supplierGrade,supplystatusList } from '@/api/capitalButtManage/intoButtList'
  import { customerType,queryCapitalEnterprise,queryCapitalEnterprise_abutment} from '@/api/common'
  ...
  export default {
    ...
    data () {
      return {
        ...
        capitalEnterpriseListData:'',//资方下拉
    ...
    mounted () {
      ...
      this.getCapitalEnterpriseListData ();//资方列表
    ...
    methods: {
      ...
      getCapitalEnterpriseListData (){
      	queryCapitalEnterprise_abutment().then(res => {
          if (res.data.code === '200') {
          	this.capitalEnterpriseListData= res.data.data
          } else {
            this.$message.error(res.data.message)
          }
        })
