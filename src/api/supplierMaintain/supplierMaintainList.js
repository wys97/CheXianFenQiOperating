 ...
 * @param status 阶段状态
 */
export function getListData ({limit, page,enterpriseName,businessLicenseNo,repreName,repreMobile,status,customerType,productName,partnerName}) {
  const data = {
    limit,
    page,
    enterpriseName,
  ...
  return fetch({
    url: test + '/admin/supply_data/list',
    method: 'post',
    data
  })
}
...
export function supplystatusList () {
  return fetch({
    url: test + '/admin/supply_data/supply_status_list',
    method: 'post'
  })
}
